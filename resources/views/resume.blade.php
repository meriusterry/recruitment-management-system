<x-app-layout>
                        ...
                        <div class="grid grid-cols-4 gap-6">
                            ...
                            <div class="bg-white p-4 shadow rounded-sm col-span-1">
                                ...
                                <form action="{{ route('applications.store') }}" method="POST">
                                    ...
                                    <button
                                        class="mt-16 bg-blue-600 text-white px-4 py-2 rounded-sm hover:bg-blue-700 w-half">
                                        Submit Application
                                    </button>
                                </form>
                                ...
                                <div class="bg-white p-4 shadow rounded-sm col-span-3 pl-4 overflow-y-scroll h-[550px]">
                                    ...
                                    <ul class="mt-4 text-black list-inside list-disc">
                                        @foreach ($data->experiences as $experience)
                                            ...
                                            <p class="text-gray-500">{{ $experience->job_title ?? '' }} :
                                    ...
                                    <ul class="mt-4 text-black list-inside list-disc">
                                        @foreach ($data->qualifications as $qualification)
                                            ...
                                            <p class="text-gray-500">{{ $qualification->institution ?? '' }} |
                                                {{ $qualification->date_completed ?? '' }}</p>
                                            <p class="text-gray-500">Graduated Date:
                                                {{ $qualification->date_completed ?? '' }}</p>
    ...
    <div id="experienceModal" class="fixed inset-0 bg-gray-700 bg-opacity-50 flex justify-center items-center hidden">
        <form method="POST" action="{{ route('cvs.store') }}"
            class="w-[600px] mx-auto bg-white p-4 shadow-lg rounded-sm">
            @csrf
            ...
            <div class="flex gap-2 mt-2 justify-end">
                ...
                <button type="button" id="closeExperienceModal"
                    class="p-2 bg-blue-500 text-white rounded">Close</button>
            </div>
    ...
    <div id="qualificationModal"
        class="fixed inset-0 bg-gray-700 bg-opacity-50 flex justify-center items-center hidden">
        <form method="POST" action="{{ route('cvs.store') }}"
            class="w-[600px] mx-auto bg-white p-4 shadow-lg rounded-sm">
            @csrf
            ...
            <input name="qualification" type="text" placeholder="Qualification Title"
            ...
            <label class="block mt-2 font-semibold">Summary</label>
            <textarea name="qualification_summary" placeholder="Qualification summary" rows="4"
                class="w-full p-2 border rounded mt-2"></textarea>

            <div class="flex gap-2 mt-2 justify-end">
                <button type="submit" class="p-2 bg-green-500 text-white rounded">Submit</button>
                <button type="button" id="closeQualificationModal"
                    class="p-2 bg-blue-500 text-white rounded">Close</button>
            </div>
    ...
    <div id="biographyModal" class="fixed inset-0 bg-gray-700 bg-opacity-50 flex justify-center items-center hidden">
        <form method="POST" action="{{ route('cvs.store') }}"
            ...
            <div class="flex gap-2 mt-2 justify-end">
                <button type="submit" class="p-2 bg-green-500 text-white rounded">Submit</button>
                <button type="button" id="closeBiographyModal"
                    class="p-2 bg-blue-500 text-white rounded">Close</button>
            </div>
    ...
    <div id="personalDetailsModal"
        ...
        <form method="POST" action="{{ route('cvs.store') }}"
            ...
            <div class="flex gap-2 mt-2 justify-end">
                <button type="submit" class="p-2 bg-green-500 text-white rounded">Submit</button>
                <button type="button" id="closePersonalDetailsModal"
                    class="p-2 bg-blue-500 text-white rounded">Close</button>
            </div>
